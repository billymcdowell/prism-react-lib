...
import * as ComponentHeroStories from '../examples/apps/sticker-sheet.stories.tsx';
...
<Story of={ComponentHeroStories.StickerSheetExample} />
...
## Docs for building production UIs

For installation, **token-first Tailwind** usage, overlay and form decisions, and a full **when-to-use** catalog of components, see the **[Production UI skill](https://billymcdowell.github.io/prism-react-lib/?path=/docs/documentation-production-ui-skill--docs)** (also shipped as `src/stories/docs/SKILL.md` for agent tools). For LLMs and scripts, the same folder is served under **`/docs/`** (Storybook `staticDirs`); see **`/docs/index.json`** for a manifest (e.g. [`/docs/button.mdx`](https://billymcdowell.github.io/prism-react-lib/docs/button.mdx)).
...
*Note: This library is currently in development (v0.0.2).*
